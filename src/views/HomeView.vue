<template>
  <div id="elem" style="background:black">  
   <div style="position:absolute ;height: 100% ; background: url('Untitled-design-35.jpg') no-repeat; background-size: 100% 100%; right: 15%;left:15%; top : 0">

    <div id="container" style=" height:550px ; float:left;mrgin-top:0px ;width:100%">

    </div>

  </div>
  <div style="position:absolute ; width:15% ; background: #223;height: 100%; background-size: 100% 100%; right: 0; top : 0; color: white">
      <div>
        <h3>
          Objects
        </h3>
        <hr>
        <div>
          <div v-for="(item, idx) in groups" v-bind:key="item" @click="selectbar(idx)" style="cursor: pointer">
            <h4 >
              {{item['name']}}
            </h4>
            <hr>
     ...
     <div class="objects" style="position:absolute ; width:15% ; background: #223;height: 100%; background-size: 100% 100%; left: 0; top : 0; color: white">
      <div>
        <h3>
          Objects
        </h3>
        <hr style="margin: 0">
        <div class="subobject">
          <div v-for="item in items" v-bind:key="item" @click="loaditem(item)" style="cursor: pointer; background: #335 ; back">
      ...
      <div class="lights">
        ...
        <hr style="margin: 0">
        <div class="sublights" @click="lightadd()"  style="cursor: pointer; background: #335 ; back">
            <h4 style="margin: 0; padding: 15px">
              روشنایی
            </h4>
            <hr>
  ...
  methods: {
    ...
    async submit(){
      ...
      await axios
        .post(`/order`, formdata)
        .then(response => {
          this.$swal('سفارش شما با موفقیت ثبت شد',
      ...
      var item = element[i]
        let objLoader = new FBXLoader();
              ...
              if(item.default.code){
                const material = new THREE.MeshPhongMaterial( {color: Number(item.default.code) , shininess : item.default.shininess} );
                object.material = material
                  ...
                  var material = new THREE.MeshPhongMaterial( { map: texture , shininess: item.default.shininess } );
                  object.material = material
                });
    ...
    setcolor(counter , code , shininess ,repeat , normal , id) {
      ...
      const material = new THREE.MeshPhongMaterial( {color: Number(code) , shininess : shininess} );
    ...
    setcolor2(counter , code , shininess , repeat , normal , id) {
      ...
      const texture2 = new THREE.TextureLoader().load(code , (texture) =>{
        ...
        var material = new THREE.MeshPhongMaterial( { map: texture2 , shininess: shininess } );
